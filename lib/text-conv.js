...
exports = module.exports = function Converter(options)
{
	...
	self.PageHeading = function(line, number, panels) {
		writeLine("PAGE " + number + " (" + panels + " panels)");
	...
	self.IssueNumber = function(line) { writeLine(" ".repeat(30) + line.trim()); }
	...
	self.Story = function(line) { writeLine(" ".repeat(30) + line.trim()); }
	self.Writer = function(line) { writeLine(" ".repeat(30) + line.trim()); }
	self.Letterer = function(line) { writeLine(" ".repeat(30) + line.trim()); }
	self.Artist = function(line) { writeLine(" ".repeat(30) + line.trim()); }
	self.Pencils = function(line) { writeLine(" ".repeat(30) + line.trim()); }
	self.Ink = function(line) { writeLine(" ".repeat(30) + line.trim()); }
	self.Color = function(line) { writeLine(" ".repeat(30) + line.trim()); }
	...
	self.Version = function(line) {
		writeLine(" ".repeat(10) + "Version: " + line.trim());
	}
	...
	self.GeneralDialog = function(dNum, name, dialog) {
		writeLine(" ".repeat(30) + dNum + "0 " + name.toUpperCase() + ":" + " ".repeat(5) + dialog);
	}

	self.CharacterName = function(line) { writeLine(" ".repeat(30) + line); }

	self.LetteringNote = function(line) {
		writeLine(" ".repeat(30) + "LETTERING NOTE:" + " ".repeat(5) + line);
	}
