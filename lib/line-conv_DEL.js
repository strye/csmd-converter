...
exports = module.exports = function ConvertLine(line, conv, mode)
{
	...
	if (line.charAt(0) === "#" && line.charAt(1) != "#") {
		if (parseVars.pageJson) {
			//console.log(parseVars.pageJson.pageNo, parseVars.pageNum)
			let panelCnt = 0;
			parseVars.pageJson.lines.forEach(element => {
				if (element.type === "panel") panelCnt++
			});
			writePage(conv.converter, mode, panelCnt)
	...
	// Default
		...
		if (line && line.length>0) parseVars.pageJson.lines.push({ type: 'desc', text: line })
	}
...
var writePage = function(conv, mode, panelCnt) {
	conv.PageHeading("", parseVars.pageJson.pageNo, panelCnt);
	parseVars.pageJson.lines.forEach(element => {
		switch (element.type) {
			case "panel":
				conv.PanelHeading("", element.number);
				break;
			case "char":
				if (mode != 'a') conv.CharacterName(element.text);
				break;
			case "dialog":
				if (mode != 'a') conv.GeneralDialog(element.number, element.name, element.text);
				break;
			case "paren":
				if (mode != 'a') conv.DialogParenthetical(element.text);
				break;
			case "letnote":
				if (mode != 'a') conv.LetteringNote(element.text);
				break;
			case "comment":
				if (mode != 'l') conv.Comment(element.text);
				break;
			case "private":
				if (mode === 'f') conv.PrivateNote(element.text);
				break;
			default:
				if (mode != 'l') conv.StandardLine(element.text);
				break;
		}				
	});
